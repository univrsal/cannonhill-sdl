...
LPDIRECTDRAWSURFACE4     lpDDSCursor	= NULL;   
LPDIRECTDRAWSURFACE4     lpDDSSchrift1	= NULL;  
LPDIRECTDRAWSURFACE4     lpDDSSchrift2	= NULL;  
LPDIRECTDRAWSURFACE4     lpDDSSchrift3	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSScape		= NULL; 
LPDIRECTDRAWSURFACE4     lpDDSHimmel	= NULL;     
LPDIRECTDRAWSURFACE4     lpDDSHimmel2	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSHimmel3	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSSchrift	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSPanzer	= NULL; 
LPDIRECTDRAWSURFACE4     lpDDSMunition	= NULL; 
LPDIRECTDRAWSURFACE4     lpDDSPanzSave	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSSonstiges = NULL;
LPDIRECTDRAWSURFACE4     lpDDSMBack		= NULL;
LPDIRECTDRAWSURFACE4     lpDDSMSBack	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSMPunkt	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSMGBack	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSMKeyboard = NULL;
LPDIRECTDRAWSURFACE4     lpDDSCredits	= NULL;
LPDIRECTDRAWSURFACE4     lpDDSTitel		= NULL;

LPDIRECTDRAWPALETTE      lpDDPal = NULL;        // DirectDraw palette
DDBLTFX					 ddbltfx;				// DirectDraw Effekte 
DDPIXELFORMAT			 ddpf;
DDSURFACEDESC2           ddsdsonstiges,ddsdback,ddsdpanzsave,ddsdscape;			//Zwischenspeicher der SurfaceEigenschaften
...
// DirectSound
LPDIRECTSOUND			lpds;			//DirectSoundPIXEL
DSBUFFERDESC            dsbdesc;
LPDIRECTSOUNDBUFFER     lpdsb;
LPDIRECTSOUNDBUFFER     lpdsbPrimary;
LPDIRECTSOUNDBUFFER     lpdsbWav[WAVANZ]; //Wavedateispeicher
LPDIRECTSOUNDBUFFER     lpdsbWavPlay[MAXCHANNELS]; //Wavedateispeicher
...
short					ButtonPush;	//Knopf gedrückt(1) oder losgelassen(-1) oder gedrückt(0) gehalten
short					MouseSoundBuffer; //Auf welchem Kanal kommt der Mousesound
struct _timeb			Zeitgenau;		//Zeit in Hunderstel
...
double					t  = 0.08;				//Wieviel Zeit vergeht in einem Berechnungsdurchgang (muß noch an die Framerate angepasst werden)
...
long					CreditsZaehler;			//Zur Zeitabhängigen Steuerung
...
ZWEID					Physik(bool touch,short InfoNr); //Verändert die in Objekt gespeicherten Wert (Bei !touch keine Berührung mit anderen Pixel)
ART						Art[ARTANZAHL];
BMP						Bmp[BILDANZ];
WAV						Wav[WAVANZ];
...
static char Schrift3[]		= "SCHRIFT3";
static char Panzerbmp[]		= "PANZERBMP";
static char Munitionbmp[]   = "MUNITIONBMP";
static char Himmelbmp[]		= "HIMMELBMP";
static char Himmel2bmp[]	= "HIMMEL2BMP";
static char Himmel3bmp[]	= "HIMMEL3BMP";
static char Sonstigesbmp[]	= "SONSTIGESBMP";
static char MBackbmp[]		= "MBACKBMP";
static char MSBackbmp[]		= "MSBACKBMP";
static char MPunktbmp[]		= "MPUNKTBMP";
static char MGBackbmp[]		= "MGBACKBMP";
static char MKeyboardbmp[]	= "MKEYBOARDBMP";
static char Creditsbmp[]	= "CREDITSBMP";
static char Titelbmp[]		= "TITELBMP";
...
void InitDDraw()
{
    ...
    pDD->QueryInterface(IID_IDirectDraw4, (LPVOID *) & lpDD);
	...
	ZeroMemory(&ddsdback, sizeof(ddsdback));
	ddsdback.dwSize = sizeof(ddsdback);
	ZeroMemory(&ddsdpanzsave, sizeof(ddsdpanzsave));
	ddsdpanzsave.dwSize = sizeof(ddsdpanzsave);
	ZeroMemory(&ddsdscape, sizeof(ddsdscape));
	ddsdscape.dwSize = sizeof(ddsdscape);

    // Create the primary surface with 1 back buffer
    ZeroMemory(&ddsdsonstiges, sizeof(ddsdsonstiges));
	ddsdsonstiges.dwSize = sizeof(ddsdsonstiges);
    ddsdsonstiges.dwFlags = DDSD_CAPS | DDSD_BACKBUFFERCOUNT;
    ddsdsonstiges.ddsCaps.dwCaps = DDSCAPS_PRIMARYSURFACE |
                          DDSCAPS_FLIP |
                          DDSCAPS_COMPLEX |
						  DDSCAPS_VIDEOMEMORY;
    ddsdsonstiges.dwBackBufferCount = 1; //Anzahl ??
    lpDD->CreateSurface( &ddsdsonstiges, &lpDDSPrimary, NULL );
    
    ddscaps.dwCaps = DDSCAPS_BACKBUFFER;
    lpDDSPrimary->GetAttachedSurface(&ddscaps, &lpDDSBack);
    	 
    ZeroMemory(&ddsdsonstiges, sizeof(ddsdsonstiges)); 
	ddsdsonstiges.dwSize = sizeof(ddsdsonstiges);  // Tell DirectDraw which members are valid. 
	ddsdsonstiges.dwFlags = DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH;  
	//In diese Surface soll die Landschaft gespeichert werden
	ddsdsonstiges.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_VIDEOMEMORY;
	ddsdsonstiges.dwWidth = MAXX;
	ddsdsonstiges.dwHeight = MAXY; 
	ddrval = lpDD->CreateSurface( &ddsdsonstiges, &lpDDSScape, NULL );
    ...
    if(ddrval != DD_OK)
    {
		...
		case DDERR_NOOVERLAYHW: break; 
		case DDERR_NOZBUFFERHW: break; 
		case DDERR_OUTOFMEMORY:  break;
		case DDERR_OUTOFVIDEOMEMORY: //{MessageBeep(MB_OK);break;}; 
		case DDERR_PRIMARYSURFACEALREADYEXISTS:break;  
		case DDERR_UNSUPPORTEDMODE:  break;
		}
	}

	DDSetColorKey(lpDDSScape, RGB(255,0,255));
	//In diese Surface soll die Schrift gespeichert werden
	/*ddsdsonstiges.dwWidth = MAXX;
	ddsdsonstiges.dwHeight = MAXY; 
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSSchrift, NULL );
	DDSetColorKey(lpDDSSchrift, RGB(255,0,255));*/
	//In diese Surface sollen die Panzer mit drumherum gespeichert werden
	ddsdsonstiges.dwWidth = MAXSPIELER*PANZERBREITE;	
	ddsdsonstiges.dwHeight = PANZERHOEHE+10; 
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSPanzSave, NULL );
	DDSetColorKey(lpDDSPanzSave, RGB(255,0,255));
	
	ddsdsonstiges.dwSize = sizeof(ddsdsonstiges);  // Tell DirectDraw which members are valid. 
    ddsdsonstiges.dwFlags = DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH;  
	ddsdsonstiges.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_SYSTEMMEMORY ; 
	
	//In diese Surface sollen die Cursor geladen werden
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSCursor, NULL ); 
	...
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSSchrift1, NULL ); 
	...
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSSchrift2, NULL ); 
	...
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSSchrift3, NULL ); 
	lpDDSSchrift3 = DDLoadBitmap(lpDD, Schrift3, 0, 0);
	DDSetColorKey(lpDDSSchrift3, RGB(255,0,255));
		
	//In diese Surface sollen die Panzer geladen werden
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSPanzer, NULL ); 
	lpDDSPanzer = DDLoadBitmap(lpDD, Panzerbmp, 0, 0);
	DDSetColorKey(lpDDSPanzer, RGB(255,0,255));
	
	//In diese Surface soll die Munition geladen werden
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSMunition, NULL ); 
	lpDDSMunition = DDLoadBitmap(lpDD, Munitionbmp, 0, 0);
	DDSetColorKey(lpDDSMunition, RGB(255,0,255));

	//Da kommt der Hintergrund rein
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSHimmel, NULL ); 
	lpDDSHimmel = DDLoadBitmap(lpDD, Himmelbmp, 0, 0);
	DDSetColorKey(lpDDSHimmel, RGB(255,0,255));

	//Da kommt der Hintergrund rein
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSHimmel2, NULL ); 
	lpDDSHimmel2 = DDLoadBitmap(lpDD, Himmel2bmp, 0, 0);
	DDSetColorKey(lpDDSHimmel2, RGB(255,0,255));

	//Da kommt der Hintergrund rein
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSHimmel3, NULL ); 
	lpDDSHimmel3 = DDLoadBitmap(lpDD, Himmel3bmp, 0, 0);
	DDSetColorKey(lpDDSHimmel3, RGB(255,0,255));
	
	//und hier die Sonstigen sachen
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSSonstiges, NULL ); 
	lpDDSSonstiges = DDLoadBitmap(lpDD, Sonstigesbmp, 0, 0);
	DDSetColorKey(lpDDSSonstiges, RGB(255,0,255));

	//hier der Menuehintergrund
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSMBack, NULL ); 
	lpDDSMBack = DDLoadBitmap(lpDD, MBackbmp, 0, 0);
	DDSetColorKey(lpDDSMBack, RGB(255,0,255));

	//hier der Shophintergrund
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSMSBack, NULL ); 
	lpDDSMSBack = DDLoadBitmap(lpDD, MSBackbmp, 0, 0);
	DDSetColorKey(lpDDSMSBack, RGB(255,0,255));

	//hier der Gewonnenhintergrund
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSMGBack, NULL ); 
	lpDDSMGBack = DDLoadBitmap(lpDD, MGBackbmp, 0, 0);
	DDSetColorKey(lpDDSMGBack, RGB(255,0,255));

	//hier die Menuepunkte
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSMPunkt, NULL ); 
	lpDDSMPunkt = DDLoadBitmap(lpDD, MPunktbmp, 0, 0);
	DDSetColorKey(lpDDSMPunkt, RGB(255,0,255));

	//hier das Keyboardbild
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSMKeyboard, NULL ); 
	lpDDSMKeyboard = DDLoadBitmap(lpDD, MKeyboardbmp, 0, 0);
	DDSetColorKey(lpDDSMKeyboard, RGB(255,0,255));

	//hier die Credits
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSCredits, NULL ); 
	lpDDSCredits = DDLoadBitmap(lpDD, Creditsbmp, 0, 0);
	DDSetColorKey(lpDDSCredits, RGB(255,0,255));

	//hier das Titelbild
	lpDD->CreateSurface( &ddsdsonstiges, &lpDDSTitel, NULL ); 
	lpDDSTitel = DDLoadBitmap(lpDD, Titelbmp, 0, 0);
	DDSetColorKey(lpDDSTitel, RGB(255,0,255));

	ddbltfx.dwSize = sizeof(ddbltfx); 
	ddpf.dwSize = sizeof(ddpf);
	lpDDSCursor->GetPixelFormat(&ddpf); //Von irgendeiner Surface die Einstellungen nehmen
}
...
void InitDSound()
{
	...
	hr = DirectSoundCreate(NULL, &lpds, NULL); //DirectSound-PIXEL machen
    if (hr != DD_OK) 
	{
		Soundzustand = -1;
		return;
    ...
    Waveformat.nChannels = 2;			//stereo oder mono (1oder2) 
...
void LoadSound(short Sound)
{
	...
	//Buffer einstellen
	dsbdesc.dwSize			= sizeof(DSBUFFERDESC);
	dsbdesc.dwFlags		=  DSBCAPS_STATIC | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPAN | DSBCAPS_CTRLFREQUENCY; //Wenn das mixen nicht funktioniert,dass hier reaktivieren
...
short PlaySound(short Sound, short Volume, short Pan, short SoundBuffer,bool Loop, short Frequenz)
{
	...
	if ((Sound == 0) || (Soundzustand <= 0)) return -1; 
	if (SoundBuffer == -1) 
	{
		...
		for (i=0;i<MAXCHANNELS;i++) 
		{
			...
			if ((lpdwStatus & DSBSTATUS_PLAYING) != DSBSTATUS_PLAYING) break;
		}
		if (i == MAXCHANNELS) return -1;
		else SoundBuffer = i;

		//Soundbuffer freigeben (ist das nötig??)
	...
	if (Pan >= MAXX) Pan = MAXX;
	if (Pan <  0)	 Pan = 0;
	z = 10000*pow((double(Pan)-double(MAXX)/2)/(double(MAXX)/2),3);
	lpdsbWavPlay[SoundBuffer]->SetPan(long(z));

	if (Frequenz != -1) lpdsbWavPlay[SoundBuffer]->SetFrequency(Frequenz);
	
	lpdsbWavPlay[SoundBuffer]->GetStatus(&lpdwStatus);
	if ((lpdwStatus & DSBSTATUS_PLAYING) != DSBSTATUS_PLAYING) 
	{
		if (Loop) lpdsbWavPlay[SoundBuffer]->Play(NULL,NULL,DSBPLAY_LOOPING); 
		else lpdsbWavPlay[SoundBuffer]->Play(NULL,NULL,NULL); 
	}
	return SoundBuffer;
}
...
{
	if ((Soundzustand <= 0) || (Channel == -1)) return;
	
	lpdsbWavPlay[Channel]->Stop();
}
...
void StopAllSound()
{
	...
	for (i=0;i<MAXCHANNELS;i++) StopSound(i);
}
...
bool LoadGame()
{
	...
	//Sound überprüfen
	if ((Soundzustand != -1) && (tmp != -1)) Soundzustand = tmp;
...
void GetMenue()
{
	...
	//Infos vom Menü auswerten
	
	//Spielereinstellungen
	for (i=1;i<=8;i++)
	{
		if (i%2 != 0) continue;
		if (Menue[MENSPIELER].getVersion(i) != 2) Panzer[i/2-1].Aktiv = true;
		else Panzer[i/2-1].Aktiv = false;
		if (Menue[MENSPIELER].getVersion(i) == 1) Panzer[i/2-1].Computer = true;
		else Panzer[i/2-1].Computer = false;
	}

	//Wind
	if (Menue[MENOPTION].getVersion(2) == 0) Windschalter = true;
	else Windschalter = false;

	//Wetter
	if (Menue[MENOPTION].getVersion(4) == 0) Wetterschalter = true;
	else Wetterschalter = false;

	//Maximale Rundenanzahl
	MaxRunde = Menue[MENOPTION].getZiffer(6);

	//Sound
	if (Menue[MENOPTION].getVersion(10) == 0) Soundzustand = 1;
...
void PutMenue()
{
	...
	{
		if (i%2 != 0) continue;
		if (Panzer[i/2-1].Aktiv == false) Menue[MENSPIELER].putVersion(i,2);
		else if (Panzer[i/2-1].Computer == true) Menue[MENSPIELER].putVersion(i,1);
		else Menue[MENSPIELER].putVersion(i,0);
	}

	//Wind
	if (Windschalter == false) Menue[MENOPTION].putVersion(2,1); 
	else Menue[MENOPTION].putVersion(2,0); 

	//Wetter
	if (Wetterschalter == false) Menue[MENOPTION].putVersion(4,1);
	else Menue[MENOPTION].putVersion(4,0);

	//Maximale Rundenanzahl
	Menue[MENOPTION].putZiffer(6,MaxRunde);

	//Sound
	if (Soundzustand <= 0) Menue[MENOPTION].putVersion(10,1);
	else Menue[MENOPTION].putVersion(10,0);
}
...
void Blitten(LPDIRECTDRAWSURFACE4 lpDDSVon,LPDIRECTDRAWSURFACE4 lpDDSNach,bool Transp)
{
	...
	{
		z++;
		hr = lpDDSNach->GetBltStatus(DDGBS_CANBLT);
 				
		if (hr == DD_OK) break;
		else if (hr == DDERR_SURFACEBUSY) break;
		Sleep(1);
...
void TranspBlt(short Bild, short Prozent)
{
	...
	Bmp[Bild].Surface->Lock(NULL,&ddsdsonstiges,DDLOCK_WAIT,NULL);
	lpDDSBack->Lock(NULL,&ddsdback,DDLOCK_WAIT,NULL);
	
	for (x=0;x<Bmp[Bild].Breite;x++)
		for (y=0;y<Bmp[Bild].Hoehe;y++)
		{
			if ((x+rcRectdes.left>=MAXX) || (x+rcRectdes.left<=0) || 
				(y+rcRectdes.top>=MAXY) || (y+rcRectdes.top<=0)) continue;
			DWORD2RGB(GetPixel((short)(x+rcRectdes.left),
					           (short)(y+rcRectdes.top),&ddsdback));
			rgbalt = rgbStruct;
			DWORD2RGB(GetPixel((short)(x+Bmp[Bild].rcSrc.left),
					           (short)(y+Bmp[Bild].rcSrc.top),&ddsdsonstiges));
...
void InitStructs(short zustand)
{
	...
	if (zustand == 0)
	{
		...
		Wav[WAVTITELMUSIK].Dateiname = "sounds\\titelmusik.wav";
		Wav[WAVTITELMUSIK].Volume    = 100;

		for (i=0;i<WAVANZ;i++) LoadSound(i);
		...
		//Panzer (die notwenigen Standardeinstellungen fürs Menü schon hier festlegen
		for (i=0;i<MAXSPIELER;i++)
		{
	...
	if ((zustand == 1) || (zustand == 2))
	{
		...
		for (i=0;i<MAXSPIELER;i++)
		{
			...
			{
				for (j=0;j<=MUNANZAHL;j++) Panzer[i].Lager[i] = 0;
		...
		Panzer[0].KeyLeft  = DIK_D;
		Panzer[0].KeyRight = DIK_F;
		Panzer[0].KeyUp    = DIK_W;
		Panzer[0].KeyDown  = DIK_S;
		Panzer[0].KeyFire  = DIK_LSHIFT;
		
		Panzer[1].Aktiv = false;
		Panzer[1].Bild	= PANZEREINS;
		Panzer[1].Farbe = RGB2DWORD(0,255,0);
		Panzer[1].Computer = false;
		Panzer[1].KeyLeft  = DIK_COMMA;
		Panzer[1].KeyRight = DIK_PERIOD;
		Panzer[1].KeyUp    = DIK_J;
		Panzer[1].KeyDown  = DIK_M;
		Panzer[1].KeyFire  = DIK_SPACE;
		
		Panzer[2].Aktiv = false;
		Panzer[2].Bild	= PANZERZWEI;
		Panzer[2].Farbe = RGB2DWORD(100,100,255);
		Panzer[2].Computer = false;
		Panzer[2].KeyLeft  = DIK_LEFT;
		Panzer[2].KeyRight = DIK_RIGHT;
		Panzer[2].KeyUp    = DIK_UP;
		Panzer[2].KeyDown  = DIK_DOWN;
		Panzer[2].KeyFire  = DIK_RETURN;
		
		Panzer[3].Aktiv = false;
		Panzer[3].Bild	= PANZERDREI;
		Panzer[3].Farbe = RGB2DWORD(255,255,0);
		Panzer[3].Computer = false;
		Panzer[3].KeyLeft  = DIK_NUMPAD4;
		Panzer[3].KeyRight = DIK_NUMPAD6;
		Panzer[3].KeyUp    = DIK_NUMPAD8;
		Panzer[3].KeyDown  = DIK_NUMPAD5;
		Panzer[3].KeyFire  = DIK_NUMPADENTER;
		...
		for (i=0;i<BERANZAHL;i++) 
			for (j=0;j<BERANZAHL;j++) BereichAktiv[i][j] = false;
...
void CheckMouse()
{
	...
	if (MousePosition.x > MAXX-Bmp[CursorTyp].Breite) MousePosition.x = MAXX-Bmp[CursorTyp].Breite;
	yDiff =	MousePosition.y;
	MousePosition.y += (short) dims.lY;
    yDiff -= MousePosition.y;
	if (MousePosition.y < 0) MousePosition.y = 0;
	if (MousePosition.y > MAXY-Bmp[CursorTyp].Hoehe) MousePosition.y = MAXY-Bmp[CursorTyp].Hoehe;
	
	Entf = short(sqrt(xDiff*xDiff+yDiff*yDiff));
	if (Entf > 10) PlaySound(WAVMOUSEEINS,
		60+Entf*40/100,
		MousePosition.x,
		-1,false,-1);
	...
	if (Testmodus)
	{
		...
		DrawString(StdString,20,10,1,lpDDSSchrift);
		sprintf(StdString, "%f", (float)Pixel[Pos.x][Pos.y].p.v.y);
		DrawString(StdString,20,30,1,lpDDSSchrift);
		sprintf(StdString, "%f", (float)Pixel[Pos.x][Pos.y].p.Relx);
		DrawString(StdString,20,50,1,lpDDSSchrift);
		sprintf(StdString, "%f", (float)Pixel[Pos.x][Pos.y].p.Rely);
		DrawString(StdString,20,70,1,lpDDSSchrift);
		sprintf(StdString, "%f", (float)Pixel[Pos.x][Pos.y].p.Art);
		DrawString(StdString,20,90,1,lpDDSSchrift);
	}
}
...
{ 
	...
	hr = g_pKey->GetDeviceState(sizeof(buffer),(LPVOID)&buffer);
	if (hr !=DI_OK) PlaySound(WAVFALLSCHIRMZWEI,100,MAXX/2,-1,true,-1);
   	
	//Überflüssige Windowsmessages abfragen und löschen (sonst beeeep nach langer Zeit)
	...
	if (Spielzustand == SZSPIEL)
	{
		...
		for (i=0;i<MAXSPIELER;i++)
		{
			...
			{
				if (KEYDOWN(buffer, Panzer[i].KeyLeft))  
				{
					MoveKanone(i,-1);
				}
				if (KEYDOWN(buffer, Panzer[i].KeyRight)) 
				{
					MoveKanone(i,+1);
				}
				if (KEYDOWN(buffer, Panzer[i].KeyUp)) 
				{
				...
				else Panzer[i].ButtonDownUp = false;
				if (KEYDOWN(buffer, Panzer[i].KeyDown))  
				{
				...
				else Panzer[i].ButtonDownDown = false;
				if ((KEYDOWN(buffer, Panzer[i].KeyFire)) && !(Panzer[i].SchussAktiv))
				{
				...
				else 
				{
					...
					if ((Panzer[i].SchussEnergie != 0) && 
						 (Panzer[i].SchussAktiv == false))	Abschuss(i);
				}
			}
		}
		...
		if ((KEYDOWN(buffer, DIK_LCONTROL)) && (KEYDOWN(buffer, DIK_PRIOR))	&& 
			(KEYDOWN(buffer, DIK_NEXT))) 
		{
			g = 9.81;
			return(1);
		}
		...
		if ((KEYDOWN(buffer, DIK_LCONTROL)) && (KEYDOWN(buffer, DIK_NEXT))) 
		{
			g--;
			if (g < 0) g = 0;
			return(1);
		}
		if ((KEYDOWN(buffer, DIK_LCONTROL)) && (KEYDOWN(buffer, DIK_HOME))) 
		{
			for (i=0;i<MAXSPIELER;i++) for (j=1;j<MUNANZAHL;j++) 
			{
				if (j != MUNSPLITTER) Panzer[i].Lager[j] = -1;
			}
			return(1);
		}
	}
	...
	{
		if ((KEYDOWN(buffer, DIK_ESCAPE)) || (KEYDOWN(buffer, DIK_SPACE)) ||
			(KEYDOWN(buffer, DIK_RETURN)) || (KEYDOWN(buffer, DIK_NUMPADENTER)))
		{
			Spielzustand = SZMENUE;
			AktMenue = MENMAIN;	
			StopAllSound();
...
void Computer()
{
	...
	for (i=0;i<MAXSPIELER;i++)
	{
		if (!(Panzer[i].Aktiv) || !(Panzer[i].Computer)) continue;
		
		if (!Panzer[Panzer[i].Ziel].Aktiv) Panzer[i].Ziel = -1;
		...
		{
			Panzer[i].Comp[Panzer[i].Ziel].Zaehler = MAXGEGNERSCHUSS; //Damit wird später ein neuer Winkel gewählt
		...
		if (Panzer[i].WunschMunition == MUNGEWEHR)
		{
			...
			if ((erg.x != -1) && (Ziel.y < Panzer[i].p.Pos.y)) //Luftlinie vorhanden
			{
				...
				if (d.x < 0) Panzer[i].WunschRichtung.x *= -1;
				Panzer[i].WunschRichtung.y = ROUND(KANONENLAENGE*sin(alpha));
				if (d.x < 0) Panzer[i].WunschRichtung.y *= -1;
			...
			else Panzer[i].WunschMunition = MUNSTEIN;
		...
		else Panzer[i].WunschRichtung.y = -1;
		...
		if (Panzer[i].Ziel == -1)
		{
			...
			while(1)
			{
				...
				if ((Panzer[i].Ziel != i) && (Panzer[Panzer[i].Ziel].Aktiv)) break;
				TestZaehler++;
				if (TestZaehler == 100) 
				{
					break;
		...
		//Richtung scheint nicht zum Erfolg zu führen
		if (Panzer[i].Comp[Panzer[i].Ziel].Zaehler >= MAXGEGNERSCHUSS)
		{
			...
			if (Panzer[i].WunschRichtung.x >= 0)
			{
				while(1)
				{
					...
					if ((tmp > 2) && (tmp <= KANONENLAENGE)) break;
					TestZaehler++;
					if (TestZaehler == 100) 
					{
						MessageBeep(MB_OK);
		...
		if ((Panzer[i].Lager[Panzer[i].WunschMunition] == 0) ||
			...
			((Panzer[i].Magnet != 0) && (Panzer[i].WunschMunition == MUNMAGNET))) //Munition wählen
		{
			Panzer[i].Comp[Panzer[i].Ziel].Zaehler++; //Damit im nächsten Durchgang hier nicht wieder rein (eigentlic falsch)
			TestZaehler = 0;
			while(1)
			{
				Panzer[i].WunschMunition = rand()%MUNANZAHL;
				if ((Panzer[i].WunschMunition == MUNMEDI) && 
					(Panzer[i].LebensEnergie >= MAXLEBENSENERGIE/2)) continue; 
				if ((Panzer[i].Lager[Panzer[i].WunschMunition] != 0)) break;
				TestZaehler++;
				if (TestZaehler == 100) 
				{
					MessageBeep(MB_OK);
		...
		if ((Panzer[i].KanonePos.y > Panzer[i].WunschRichtung.y+1) &&
			(Panzer[i].WunschRichtung.y != -1))
		{
			if (Panzer[i].KanonePos.x < 0)	MoveKanone(i,+1);
			else MoveKanone(i,-1);
			continue;
		}
...
void CalcSchussErgebnis(short x, short Besitzer)
{
	...
	if (Panzer[Besitzer].Computer == false) return;

	if (Panzer[Besitzer].p.Pos.x > Panzer[Panzer[Besitzer].Ziel].p.Pos.x)
	{
		if (x < Panzer[Panzer[Besitzer].Ziel].p.Pos.x) Zuweittmp = true;
		else Zuweittmp = false;
	}
	else
	{
		if (x > Panzer[Panzer[Besitzer].Ziel].p.Pos.x) Zuweittmp = true;
...
{
	if ((x >= MAXX) || (x < 0) || (y >= MAXY) || (y < 0)) return;


    WORD *pixels = (WORD *)ddsdtmp->lpSurface;
  	DWORD pitch = ddsdtmp->lPitch>>1;
	...
	else
	{
		...
		for (x1=(short)(x/(MAXX/BERANZAHL))*(MAXX/BERANZAHL);x1<((short)(x/(MAXX/BERANZAHL))+1)*(MAXX/BERANZAHL); x1++)
			for (y1=(short)(y/(MAXY/BERANZAHL))*(MAXY/BERANZAHL);y1<((short)(y/(MAXY/BERANZAHL))+1)*(MAXY/BERANZAHL); y1++)
				if (Pixel[x1][y1].Aktiv) check = true;
		if (!check)		
...
ZWEID Physik(bool touch,short InfoNr)
{
	...
	{
		//Haftung (hauptsächlich, um den bescheuerten Fehler zu beseitigen)
	...
	else if (yneu < 0)
	{
		if (InfoNr < 0)
		{
			for (i=0;i<MAXHIMMELPIXEL;i++)
			{
				if (HimmelPixel[i].Art != ARTNICHTS) continue;
	...
	if ((Pixel[xneu][yneu].p.Art != ARTNICHTS) && ((xneu != Objekt.Pos.x) || (yneu != Objekt.Pos.y))
		&& (touch))
	{
...
void CheckMunListe()
{
	...
	for (i=0;i<MAXMUN;i++)
	{
		if (MunListe[i].Besitzer == -1) continue;
	
		if (Bild%(LastBild/30+1) == 0) MunListe[i].p.Lifetime--;
		...
		//Magnetrakete
		if (MunListe[i].p.Art == MUNMAGRAKETE) MakeMagRakete(); //benutzt das 'objekt'
		
		if (MunListe[i].Ausserhalb == true) erg = Physik(false,i);
		else erg = Physik(true,i);
		if ((erg.x == -1) && (erg.y == -1)) continue;
			 ...
			 (MunListe[i].p.Lifetime == 0) || Objekt.Kontakt) 
		{
			...
			if (Treffer == -2) BallonAbschuss(tmp);
		}
		...
		//Smoke
		if (Bild%(LastBild/50+1) == 0)	//unabhänig von der Framerate
		{
			for (k=0; k<2; k++)
			{
...
void CheckPanzer()
{
	...
	for (i=0;i<MAXSPIELER;i++)
	{
		...
		//Magnet?
		if (Panzer[i].Magnet > 0) MakeMagnet(i);

		//Abflackeffekte
		if (Bild%(LastBild/20+1) == 0)	//unabhängig von der Framerate
		{
...
void CheckHimmelPixel()
{
	...
	for (i=0;i<MAXHIMMELPIXEL;i++)
	{
		...
		erg = Physik(false,-1);
		//Wenn außerhalb vom berechenbaren Bereich
		if ((erg.x == -1) && (erg.y == -1)) 
		{
			HimmelPixel[i].Art = ARTNICHTS;
			continue;
		...
		if (FXListe[i].Lifetime > 0) 
		{
			if ((FXListe[i].Bewegung.x != 0) || (FXListe[i].Bewegung.y != 0))
			{
				...
				{	
					...
					{
						//Wenn nicht, Pixel löschen
						PutPixel(FXListe[i].Pos.x,FXListe[i].Pos.y,Tansparent,&ddsdscape);
			...
			if (i <= LastFX) PutPixel(FXListe[i].Pos.x,FXListe[i].Pos.y,
									  FXListe[i].Farbe,&ddsdscape);
		}
		else 
		{
			PutPixel(FXListe[i].Pos.x,FXListe[i].Pos.y,Tansparent,&ddsdscape);
...
void MakeFXPixel(short x, short y, DWORD Farbe, short Lifetime, double Bewx, double Bewy)
{
	...
	for (j=0;j<=LastFX;j++)
	{
		if ((FXListe[j].Pos.x != -1) && (FXListe[j].Pos.y != -1)) continue;
...
void CheckBallon()
{
	...
	if ((Ballon.Pos.x >= MAXX) || (Ballon.Pos.x+Bmp[BALLONBILD].Breite < 0) ||
		(Ballon.Pos.y >= MAXY)) Ballon.Aktiv = false;
}

void MoveKanone(short i,short Richtung)
{
	ZWEID NewPos;
	double Div1,Div2; //Unterschied zur richtigen Länge

	NewPos = Panzer[i].KanonePos;

	if (Richtung >= 0) NewPos.x++;
	else NewPos.x--;
	if (Panzer[i].KanonePos.x >= 0)
	{
		if (Richtung >= 0) NewPos.y++;
		else NewPos.y--;
	}
	else
	{
	...
	Div1 = sqrt(NewPos.x*NewPos.x + Panzer[i].KanonePos.y*Panzer[i].KanonePos.y)
		  - KANONENLAENGE;
	Div2 = sqrt(NewPos.y*NewPos.y + Panzer[i].KanonePos.x*Panzer[i].KanonePos.x)
		  - KANONENLAENGE;
	if (Div1 < 0) Div1 = -Div1;
	if (Div2 < 0) Div2 = -Div2;
	
	if (Div1 <= Div2) Panzer[i].KanonePos.x = NewPos.x;
	else Panzer[i].KanonePos.y = NewPos.y;
	if (Panzer[i].KanonePos.y > -2) Panzer[i].KanonePos.y = -2;
	ZeichnePanzer(i,0);
}
...
void Abschuss(short i)
{
		 ...
		 (Panzer[i].Lager[Panzer[i].Munition] == -1)))
	{
		...
		else
		{
			for (j=0;j<MAXMUN;j++)
			{
				if (MunListe[j].Besitzer != -1) continue;
...
void ChangeLebensEnergie(short i,short Anzahl,short Verursacher)
{
	...
	if (Panzer[i].LebensEnergie > MAXLEBENSENERGIE) Panzer[i].LebensEnergie = MAXLEBENSENERGIE;

	if (Panzer[i].LebensEnergie < 0) 
	{
		if ((Verursacher != -1) && (Verursacher != i)) Panzer[Verursacher].Abschuesse++;
	...
	if (ChangeVersion) ZeichnePanzer(i,0);
...
void ChangeMunition(short i, short add)
{
	...
	if (Panzer[i].Munition >= MUNANZAHL) Panzer[i].Munition = 0;
	else if (Panzer[i].Munition < 0) Panzer[i].Munition = MUNANZAHL-1;
	if (Panzer[i].Lager[Panzer[i].Munition] == 0) ChangeMunition(i,add);
	ZeichnePanzer(i,1);
}
...
short CheckKollision(short x,short y, bool genau)
{
	...
	for (i=0;i<MAXSPIELER;i++)		//Panzer
	{
		...
		if ((x >= Panzer[i].p.Pos.x-Bmp[Panzer[i].Bild].Breite/2) &&
			...
			(y < Panzer[i].p.Pos.y))
		{
			if (genau) //Für genaue Abfrage erst die Surface locken!!
			{
				...
				yintern = Bmp[Panzer[i].Bild].rcSrc.top+
					y-(Panzer[i].p.Pos.y-Bmp[Panzer[i].Bild].Hoehe);
				//Treffer?
				if (GetPixel(xintern,yintern,&ddsdsonstiges) != Tansparent) 
				{
					if (Testmodus) PutPixel(xintern,yintern,RGB2DWORD(0,0,255),&ddsdsonstiges);
					erg = i;
					break;
				}
			}
	...
	if (erg == -1)
	{
		...
		{
			if (MunListe[i].Besitzer == -1) continue;
			if ((MunListe[i].p.Pos.x == x) && (MunListe[i].p.Pos.y == y)) continue;
			//Grobe Einkreisung 
...
void FindActivePixel()
{
	...
	for (i=0;i<MAXSPIELER;i++) PanzerDamage[i] = 0;

	CheckNr = rand()%1000; //Checknr damit jeder Pixel nur einmal im Durchgang berechnet wird
	
	for (i=0;i<BERANZAHL;i++) for (j=0;j<BERANZAHL;j++)
	{
		if ((!BereichAktiv[i][j]) && (!Testmodus)) continue;
		for (x=i*(MAXX/BERANZAHL);x<(i+1)*(MAXX/BERANZAHL); x++)
			for (y=j*(MAXY/BERANZAHL);y<(j+1)*(MAXY/BERANZAHL); y++)				
			{
				if ((!Pixel[x][y].Aktiv) || (Pixel[x][y].CheckNr == CheckNr) ||
					(Pixel[x][y].p.Art == ARTNICHTS)) continue;
				
				if (Testmodus) 
				{
					if (Pixel[x][y].Aktiv) PutPixel(x,y,RGB2DWORD(0,255,0),&ddsdscape);  
					else PutPixel(x,y,RGB2DWORD(255,0,0),&ddsdscape);
				}
				...
				//Pixel mit beschränkter Lebensdauer löschen
				if (Bild%(LastBild/10+1) == 0)		//unabhänig von der Framerate
				{	
					if (Pixel[x][y].p.Lifetime > 0) Pixel[x][y].p.Lifetime--;
				...
				//Berührung der Säure mit Panzer?
				tmp = -1;
				...
				if ((sqrt(Pixel[x][y].p.v.x*Pixel[x][y].p.v.x+
					...
					(erg.x == x) && (erg.y == y) &&
					(Pixel[erg.x][erg.y+1].p.Art != ARTNICHTS) &&
					(Pixel[x][y].p.Lifetime == -1)) Aktivate(x,y,false); //Anhalten wenn zu langsam
				else
				{
		...
		if ((Testmodus) && (BereichAktiv[i][j]))
			PutPixel(i*(MAXX/BERANZAHL),j*(MAXY/BERANZAHL),RGB2DWORD(0,0,255),&ddsdscape);
	}
	//Energie abziehen
	for (i=0;i<MAXSPIELER;i++) ChangeLebensEnergie(i,-PanzerDamage[i],-1);
}
...
{	
	...
	if ((x1 >= MAXX) || (x1 < 0) || (y1 >= MAXY) || (y1 < 0)) return erg;
	if ((x2 >= MAXX) || (x2 < 0) || (y2 >= MAXY) || (y2 < 0)) return erg;
	...
	for (i=0; i<Steps; i++) 
	{
		...
		if (gesamt) //Für den Computergegner
		{
			if (Pixel[(short)(x)][(short)(y)].p.Art != ARTNICHTS)
			{
				erg.x = -1;
...
void MovePixel(short x, short y, short xneu, short yneu)
{
	...
	if ((x >= MAXX) || (x < 0) || (y >= MAXY) || (y < 0)) return;
	if ((xneu >= MAXX) || (xneu < 0) || (yneu >= MAXY) || (yneu < 0)) return;
	...
	if (!Testmodus)
	{
		PutPixel(x,y,Pixel[x][y].Farbe,&ddsdscape); 
	}
	Aktivate(x,y,false);
	//Alle Pixel ausenherum aktivieren
	if  (x-1 >= 0)				       if (Pixel[x-1][y].p.Art != ARTNICHTS) Aktivate(x-1,y,true);
	if ((x-1 >= 0)    && (y-1 >= 0))   if (Pixel[x-1][y-1].p.Art != ARTNICHTS) Aktivate(x-1,y-1,true);
	if				     (y-1 >= 0)    if (Pixel[x][y-1].p.Art != ARTNICHTS) Aktivate(x,y-1,true);
	if ((x+1 < MAXX)  && (y-1 >= 0))   if (Pixel[x+1][y-1].p.Art != ARTNICHTS) Aktivate(x+1,y-1,true);
	if  (x+1 < MAXX)                   if (Pixel[x+1][y].p.Art != ARTNICHTS) Aktivate(x+1,y,true);
	if ((x+1 < MAXX)  && (y+1 < MAXY)) if (Pixel[x+1][y+1].p.Art != ARTNICHTS) Aktivate(x+1,y+1,true);
	if                   (y+1 < MAXY)  if (Pixel[x][y+1].p.Art != ARTNICHTS) Aktivate(x,y+1,true);
	if ((x-1 >= 0)    && (y+1 < MAXY)) if (Pixel[x-1][y+1].p.Art != ARTNICHTS) Aktivate(x-1,y+1,true);
	...
	if (!Testmodus)
	{
		PutPixel(xneu,yneu,Pixel[xneu][yneu].Farbe,&ddsdscape);  //Pixel malen
	}
...
void MakePixel(short x, short y, short Typ,short vx, short vy, bool Active, long Lifetime)
{
	...
	Pixel[x][y].p.Art = Typ;
	if (Pixel[x][y].p.Art == ARTNICHTS) return;
	Pixel[x][y].p.v.x     = vx;
	Pixel[x][y].p.v.y     = vy;
	Pixel[x][y].p.Lifetime  = Lifetime;
	if (Active) Aktivate(x,y,true);
	if (Typ != ARTSTEIN)
	{
	...
	{
		Bmp[TEXFELS].Surface->Lock(NULL,&ddsdsonstiges,DDLOCK_WAIT,NULL);
		Pixel[x][y].Farbe = GetPixel(x%Bmp[TEXFELS].Breite,
									 y%Bmp[TEXFELS].Hoehe,&ddsdsonstiges);
		Bmp[TEXFELS].Surface->Unlock(NULL);
	}
	
	PutPixel(x,y,Pixel[x][y].Farbe,&ddsdscape);  //Pixel in die Surface malen
}
...
double Steigung(short x, short y)
{
	...
	else if ((Pixel[x+1][y+1].p.Art == ARTNICHTS) || (Pixel[x-1][y+1].p.Art == ARTNICHTS))
	{
		for (y1=y;y1<=MAXY;y1++)
		{
			if (Pixel[x-1][y1].p.Art != ARTNICHTS) break;
		}
		for (y2=y;y2<=MAXY;y2++)
		{
			if (Pixel[x+1][y2].p.Art != ARTNICHTS) break;
		}
		if (y1>=y2)
		{
			xdiv = -1;
	...
	else
	{
		for (x1=x-1;x1>=0;x1--)
		{
			...
			if (Pixel[x1][y+1].p.Art == ARTNICHTS) break; 
		}
		for (x2=x+1;x2<MAXX;x2++)
		{
			...
			if (Pixel[x2][y+1].p.Art == ARTNICHTS) break; 
		}
		if (x1 == -1)
		{
			if (x2 == -1) 
	...
	if (erg < 0) erg = 0; //Sicherheitsabfrage
	return(erg);			
}
...
void MakeMagnet(short i)
{
	...
	for (j=0;j<MAXMUN;j++)
	{
		if ((MunListe[j].Besitzer == -1) ||
			...
			(MunListe[j].p.Art == MUNMEGA)) continue;
...
void MakeWetter()
{
	if (Bild%(LastBild/10+1) != 0)	return;	//unabhänig von der Framerate
...
void NeuesSpiel()
{
	...
	//Windbeschleunigung
	if (Windschalter) Wind.x	= rand()%7-3;
	else Wind.x = 0;
	Wind.y	   = 0;

	//Wetter
	if (Wetterschalter) Wetter	= rand()%3;
	else Wetter = 0;
	...
	rcRectdes.right		= MAXX;
	rcRectdes.bottom	= MAXY;
	lpDDSPrimary->Blt(&rcRectdes,lpDDSMBack,&rcRectdes,DDBLT_WAIT,NULL);
	...
	lpDDSPrimary->Blt(&rcRectdes,Bmp[MLADESPIEL].Surface,&rcRectsrc,DDBLT_WAIT,NULL);
	rcRectdes.left		= 0;
	rcRectdes.top		= 0;
	rcRectdes.right		= MAXX;
	rcRectdes.bottom	= MAXY;
	...
	//Scapesurface löschen
	lpDDSScape->Blt(&rcRectdes, NULL,NULL,DDBLT_COLORFILL, &ddbltfx);
	//PanzSavesurface löschen
	rcRectdes.left		= 0;
	rcRectdes.top		= 0;
	rcRectdes.right		= MAXSPIELER*PANZERBREITE;
	rcRectdes.bottom	= PANZERHOEHE;
	ddbltfx.dwFillColor = Tansparent;
	lpDDSPanzSave->Blt(&rcRectdes, NULL,NULL,DDBLT_COLORFILL, &ddbltfx);
	...
	*/
	...
	ZeichneBmp(0,PANZERHOEHE,WINDHOSE,rcGesamt,(short)Wind.x+3,lpDDSPanzSave); 
}
...
void InitCredits()
{
	...
	//Boden einbauen
	lpDDSScape->Lock(NULL,&ddsdscape,DDLOCK_WAIT,NULL);
	for (x=0;x<MAXX;x++) for (y=MAXY-2;y<MAXY;y++) 
...
void CheckCredits()
{
	...
	for (i=0;i<MAXCREDITS;i++)
	{
		...
		if (CreditsListe[i].Zeit == CreditsZaehler) break;
		if (CreditsListe[i].Zeit > CreditsZaehler) return;
	}
	
	if (CreditsListe[i].Bild == -2)
	{
		for (x=0;x<MAXX;x++) for (y=20;y<68;y++) Aktivate(x,y,true);
		return;
	}
	if (CreditsListe[i].Bild == -3)
	{
		for (x=0;x<MAXX;x++) for (y=MAXY/2-24;y<MAXY/2+24;y++) Aktivate(x,y,true);
		return;
	}

	Bild = CreditsListe[i].Bild;
	if (CreditsListe[i].Ueberschrift) 
		for (x=0;x<MAXX;x++) for (y=20;y<68;y++) Aktivate(x,y,false);
	else for (x=0;x<MAXX;x++) for (y=MAXY/2-24;y<MAXY/2+24;y++) Aktivate(x,y,false);
		
		
	if (CreditsListe[i].Ueberschrift) y = 20;
	else y = MAXY/2-Bmp[Bild].Hoehe/2;
	...
	Bmp[Bild].Surface->Lock(NULL,&ddsdback,DDLOCK_WAIT,NULL); //Eigentlich hier ddsdsonstiges
	lpDDSScape->Lock(NULL,&ddsdscape,DDLOCK_WAIT,NULL);
	
	for (Relx=0;Relx<Bmp[Bild].Breite;Relx++)
		for (Rely=0;Rely<Bmp[Bild].Hoehe;Rely++)
		{
			Farbe = GetPixel(Bmp[Bild].rcSrc.left+Relx,Bmp[Bild].rcSrc.top+Rely,&ddsdback);
			if (Farbe == RGB2DWORD(255,255,255)) continue;
			if (CreditsListe[i].Ueberschrift) 
				MakePixel(x+Relx,y+Rely,Material,0,0,false,-1);
			else MakePixel(x+Relx,y+Rely,Material,0,0,false,-1);
		}
	Bmp[Bild].Surface->Unlock(NULL);
	lpDDSScape->Unlock(NULL);
}
...
void Zeige(bool flippen)
{
	...
	if ((Spielzustand == SZCREDITS) || (Testmodus)) Himmel = 1;

	//Hintergrund zeichnen
	if (Himmel == 0) Blitten(lpDDSHimmel,lpDDSBack,false); 
	else if (Himmel == 1) Blitten(lpDDSHimmel2,lpDDSBack,false);
	else if (Himmel == 2) Blitten(lpDDSHimmel3,lpDDSBack,false); 
	

	Blitten(lpDDSScape,lpDDSBack,true); //Landschaft zeichnen
	...
	if (Ballon.Aktiv)
	{
		ZeichneBmp(Ballon.Pos.x,Ballon.Pos.y,BALLONBILD,rcGesamt,0,lpDDSBack);
		if ((Ballon.Munition == MUNGEWEHR) || (Ballon.Munition == MUNFASS))
		{
			ZeichneBmp(Ballon.Pos.x+11,
				Ballon.Pos.y+42,
				Munition[Ballon.Munition].Bild,rcGesamt,8,lpDDSBack);
		}
		else ZeichneBmp(Ballon.Pos.x+11,
			Ballon.Pos.y+42,
			Munition[Ballon.Munition].Bild,rcGesamt,0,lpDDSBack);
		sprintf(StdString, "%d", Ballon.Anzahl);
		DrawString(StdString,Ballon.Pos.x+10,Ballon.Pos.y+42,2,lpDDSBack);
	}
	...
	if ((Spielzustand == SZSPIEL) && (Wind.x != 0))
	{
		...
		Blitten(lpDDSPanzSave,lpDDSBack,false); 
	}
...
void MakeTitel()
{
	...
	rcRectdes.right		= MAXX;
	rcRectdes.bottom	= MAXY;
	Blitten(lpDDSTitel,lpDDSPrimary,false);
...
void ZeigePanzer()
{
	...
	for(i=0;i<MAXSPIELER;i++)
	{
		...
		CalcRect(rcGesamt);
		Blitten(lpDDSPanzSave,lpDDSBack,true);	//Panzer zeichnen

		if (Panzer[i].Schild > 0)
		{
			if (Bild%(LastBild/30+1) == 0) Panzer[i].Schild--;
			if (Bild%2 == 0) 
				ZeichneBmp(Panzer[i].p.Pos.x-15,Panzer[i].p.Pos.y-14,SCHILD,rcGesamt,0,lpDDSBack);
		}
		if (Panzer[i].Magnet > 0)
		{
			if (Bild%(LastBild/30+1) == 0) Panzer[i].Magnet--;
			ZeichneBmp(Panzer[i].p.Pos.x-5,Panzer[i].p.Pos.y-13,MAGNET,rcGesamt,0,lpDDSBack);
		}
	}
}
	...
	for (i=0;i<MAXMUN;i++)
	{
		...
		if (MunListe[i].Ausserhalb == false)
		{
			...
			else
			{
				...
				if (x>=0)
				{
					if (Grad <= -3*pi/8) Richtung = 6;
					else if (Grad <= -pi/8) Richtung = 7;
					else if (Grad <= +pi/8) Richtung = 0;
					else if (Grad <= +3*pi/8) Richtung = 1;
					else Richtung = 2;
				}
				else
				{
					if (Grad <= -3*pi/8) Richtung = 2;
					else if (Grad <= -pi/8) Richtung = 3;
					else if (Grad <= +pi/8) Richtung = 4;
					else if (Grad <= +3*pi/8) Richtung = 5;
					else Richtung = 6;
				}
			}
			ZeichneBmp(MunListe[i].p.Pos.x-Bmp[Munition[MunListe[i].p.Art].Bild].Breite/2,
				...
				Munition[MunListe[i].p.Art].Bild, rcGesamt,Richtung,lpDDSBack);
		}
		else
		{
			lpDDSBack->Lock(NULL,&ddsdback,DDLOCK_WAIT,NULL);	 
			...
			ZeichneLinie(MunListe[i].p.Pos.x-5,0,MunListe[i].p.Pos.x+5,0,
					Panzer[MunListe[i].Besitzer].Farbe,&ddsdback);
			lpDDSBack->Unlock(NULL); 
		}
	}
}
...
void ZeichnePanzer(short i,short Teil)
{
	...
	case 0:
		...
		lpDDSPanzSave->Lock(NULL,&ddsdpanzsave,DDLOCK_WAIT,NULL);
		//Kanone
		ZeichneLinie(PanzSavPos[i].x+Bmp[Panzer[i].Bild].Breite/2, 
			...
			Panzer[i].Farbe,&ddsdpanzsave);
		lpDDSPanzSave->Unlock(NULL); 

		//Panzer
		ZeichneBmp(PanzSavPos[i].x,
				   PanzSavPos[i].y-Bmp[Panzer[i].Bild].Hoehe,
			Panzer[i].Bild, rcGesamt,Panzer[i].Version,lpDDSPanzSave);
		break;
	case 1:
		//Munitionsbox
		ZeichneBmp(PanzSavPos[i].x,
				   PanzSavPos[i].y+2,
				MUNBOX,rcGesamt,0,lpDDSPanzSave);
		...
		if (Panzer[i].SchussEnergie != 0)
		{
			...
			CalcRect(rcGesamt);
			Blitten(Bmp[POWER].Surface,lpDDSPanzSave,false);
		}
		//SchussEnergieschrift
		sprintf(StdString, "%d", Panzer[i].SchussEnergie);
		DrawString(StdString,PanzSavPos[i].x+4,PanzSavPos[i].y+22,2,lpDDSPanzSave);
		...
		CalcRect(rcGesamt);
		Blitten(Bmp[ENERGIE].Surface,lpDDSPanzSave,false);
		//Munition
		if ((Panzer[i].Munition == MUNGEWEHR) || (Panzer[i].Munition == MUNFASS))
		{
			ZeichneBmp(PanzSavPos[i].x+7,
				PanzSavPos[i].y+5,
				Munition[Panzer[i].Munition].Bild,rcGesamt,8,lpDDSPanzSave);
		}
		else ZeichneBmp(PanzSavPos[i].x+7,
				PanzSavPos[i].y+5,
				Munition[Panzer[i].Munition].Bild,rcGesamt,0,lpDDSPanzSave);
		//Munitionsanzahl
		if (Panzer[i].Lager[Panzer[i].Munition] >= 0)
		{
			...
			DrawString(StdString,PanzSavPos[i].x+5,
				PanzSavPos[i].y+4,2,lpDDSPanzSave);
		}
		break;
	case 2:
		//Panzer löschen
...
void ZeichneBmp(short x, short y, short i, RECT Ziel, short Version,LPDIRECTDRAWSURFACE4 lpDDSNach)
{
	rcRectsrc = Bmp[i].rcSrc;
	rcRectsrc.top += Bmp[i].Hoehe*Version;
	...
	CalcRect(Ziel);
	Blitten(Bmp[i].Surface,lpDDSNach,true);
}
...
void DrawString(char *string, short x, short y, short Art,LPDIRECTDRAWSURFACE4 lpDDSNach)
{
	short length, index, cindex,Breite,Hoehe;
	
	if (lpDDSNach == lpDDSSchrift) return; //Für Fehlerausgabe aktivieren
	...
	//Schriftsurface löschen (Noch nicht entgültig)
	if (lpDDSNach==lpDDSSchrift)
	{
		rcRectdes.left		= x-5;
		rcRectdes.top		= y-5;
	...
	for (index=0; index<length; index++)
	{
		...
		if (Art == 1) 
		{
			Blitten(lpDDSSchrift1,lpDDSNach,true);
			//x Position weiterschieben
			x+=S1ABSTAND;
		}
		if (Art == 2) 
		{
			Blitten(lpDDSSchrift2,lpDDSNach,true);
			//x Position weiterschieben
			x+=S2ABSTAND;
		}
		if (Art == 3) 
		{
			Blitten(lpDDSSchrift3,lpDDSNach,true);
			//x Position weiterschieben
			x+=S3ABSTAND;
		}
	} 
...
void Compute() //Gebirge generieren
{
	...
	while(1)
	{
		...
		for (i=1;i<=Anzahl-1;i++)
		{
			...
			if (i == 1) Punkt[2*i].y = (Punkt[2*i-1].y+Punkt[2*i+1].y)/2-rand()%(Abstand/2);
			else  Punkt[2*i].y = (Punkt[2*i-1].y+Punkt[2*i+1].y)/2-rand()%(Abstand)+Abstand/2;
			//zu tief oder zu hoch
			if (Punkt[2*i].y > MAXY-50) Punkt[2*i].y = MAXY-50;
			if (Punkt[2*i].y < 100) Punkt[2*i].y = 100;
		}
		Anzahl = 2*Anzahl-1;
		if (2*Anzahl-1>MaxPunkte) break;
	}
	//Landschaft auffüllen
	i = 2;
	for (x=0;x<MAXX;x++)
	{
		if (x==Punkt[i].x) i++;
...
void PutPanzer()
{
	...
	Spieleranz = 0;
	for (i=0;i<MAXSPIELER;i++) if (Panzer[i].Aktiv) Spieleranz++;
	Nichtspieler = 0;
	for (i=0;i<=MAXSPIELER;i++)
	{
		if (!Panzer[i].Aktiv) 
...
void Explosion(short MunNr)
{
	...
	short DamageBetrag;			//Wie groß ist die Beschädigung

	if (Munition[MunListe[MunNr].p.Art].Explosion == 0) return;
	...
	if (Munition[MunListe[MunNr].p.Art].Explosion == EXPKRATER)
	{
		...
		for (Relx=-10;Relx<=10;Relx++) for (Rely=-10;Rely<=10;Rely++)
		{
			if ((Relx*Relx+Rely*Rely) > (10*10)) continue;
			x = MunListe[MunNr].p.Pos.x+Relx;
			y = MunListe[MunNr].p.Pos.y+Rely;
			if ((x >= MAXX) || (x < 0) || (y >= MAXY) || (y < 0)) continue;
			if (Pixel[x][y].p.Art != ARTNICHTS) 
			{
				if (rand()%2 == 0) DeletePixel(x,y);
				else
				{
	...
	else if (Munition[MunListe[MunNr].p.Art].Explosion == EXPEXPLOSION)
	{
		...
		for (Relx=-20;Relx<=20;Relx++) for (Rely=-20;Rely<=20;Rely++)
		{
			if ((Relx*Relx+Rely*Rely) > (20*20)) continue;
			x = MunListe[MunNr].p.Pos.x+Relx;
			y = MunListe[MunNr].p.Pos.y+Rely;
			if ((x >= MAXX) || (x < 0) || (y >= MAXY) || (y < 0)) continue;
			if (Pixel[x][y].p.Art != ARTNICHTS) 
	...
	else if (Munition[MunListe[MunNr].p.Art].Explosion == EXPSPLITTER)
	{
		...
		for (Relx=-5;Relx<=5;Relx++) for (Rely=-5;Rely<=5;Rely++)
		{
			if ((Relx*Relx+Rely*Rely) > (5*5)) continue;
			x = MunListe[MunNr].p.Pos.x+Relx;
			y = MunListe[MunNr].p.Pos.y+Rely;
			if ((x >= MAXX) || (x < 0) || (y >= MAXY) || (y < 0)) continue;
			...
			for (j=0;j<MAXMUN;j++)
			{
				...
				while(1)
				{
					...
					if ((tmp == MUNRAKETE) || (tmp == MUNSTEIN) || (tmp == MUNGRANATE) ||
						(tmp == MUNMAGRAKETE) || (tmp == MUNFASS)) break;
	...
	//Energie abziehen
	for (i=0;i<MAXSPIELER;i++) 
		if (Panzer[i].Schild == 0) ChangeLebensEnergie(i,-PanzerDamage[i],MunListe[MunNr].Besitzer);
}
...
void Fetz(short x, short y, short Bild)
{
	...
	for (Relx=0;Relx<Bmp[Bild].Breite;Relx++)
		for (Rely=0;Rely<Bmp[Bild].Hoehe;Rely++)
		{
			Farbe = GetPixel(Bmp[Bild].rcSrc.left+Relx,Bmp[Bild].rcSrc.top+Rely,&ddsdsonstiges);
			if (Farbe == Tansparent) continue;
...
void Laser(short i)
{
	...
	k = 1;
	BallonTreffer = false;
	for (j=0;j<MAXSPIELER;j++) Treffer[j] = false;
	while(1)
	{
		...
		for (j=0; j<Steps; j++) 
		{
			...
			if ((erg >= 0) && (erg < MAXSPIELER)) Treffer[erg] = true; 
			else if (erg == -2) BallonTreffer = true;
			x += Sx;
			y += Sy;
		}
		k++;
	...
	if (BallonTreffer) BallonAbschuss(i);
}
...
void BallonAbschuss(short i)
{
	...
	if (i != -1)
	{
		...
		if (Panzer[i].Lager[Ballon.Munition] == -1) return;
		Panzer[i].Lager[Ballon.Munition] += Ballon.Anzahl;
		if (Panzer[i].Lager[Ballon.Munition] > 9) Panzer[i].Lager[Ballon.Munition] = 9;
	}
	else PlaySound(WAVSAEURE,100,Ballon.Pos.x,-1,false,-1);	
	//Explosion
...
void CheckShop(short i)
{
	...
	while(1)
	{
		...
		{
			k=erg/6;
			if (Panzer[i].Lager[k+1] == -1) continue;
	...
	while(1)
	{
		...
		if (AktMenue == MENOPTION)
		{
			if (erg==7)	
			{
				...
				i++;
				if (i <= 9) Menue[AktMenue].putZiffer(6,i);
			...
			else if (erg==8)	
			{
				i = Menue[AktMenue].getZiffer(6);
				i--;
				if (i >= 1) Menue[AktMenue].putZiffer(6,i);
			}
			else if (erg==10) 
			{
				if (Soundzustand == -1) Menue[AktMenue].putVersion(erg,1);
				else if (Menue[AktMenue].getVersion(erg) == 0) Soundzustand = 1;
				else Soundzustand = 0;
			}
		}
		else if (AktMenue == MENSPIELER)
		{
			...
			{
				anz = 0;
				for (i=2;i<=8;i+=2) if (Menue[AktMenue].getVersion(i) == 2) anz++;
				if (anz >= 3) Menue[AktMenue].putVersion(erg,0);
			}
		}
		Menue[AktMenue].zeige();			//Menue anzeigen
	}
...
void ComputerShop(short i)
{
	...
	for (j=0;j<100;j++)
	{
		...
		if (Panzer[i].Lager[k] >= 9) continue;
		if (Panzer[i].Konto < Munition[k].Preis) continue;
		if (k == MUNSTEIN) continue;
...
short Refresh()
{
	...
	while(1)
	{
		...
		//Auf bestimmte fps beschränken
		Zeitdiv = Zeitgenautmp.millitm-Zeitgenau.millitm;
		if (Zeitdiv < 0)
			Zeitdiv = 1000+Zeitdiv;
		Zeitcheck = 10-(Zeitdiv);
		t = (((float)Zeitdiv / 250)+t*19)/20;
		
		//fps rausfinden
		Bild++;
		if (Zeitgenau.time < Zeitgenautmp.time)
		{
			LastBild = Bild;
			Bild = 1;
			if (LastBild==1) LastBild = 25; 
			
			//BilderproSec ausgeben
			sprintf(StdString, "%d", LastBild);
			DrawString(StdString,5,5,2,lpDDSSchrift);
		}
		Zeitgenau = Zeitgenautmp;

		if (Spielzustand == SZSPIEL)
		{
			if (AktMenue == -1)					//Kein Menü eingeblendet (normal)
			{
				...
				if (Bild%(LastBild/30+1) == 0)		//unabhänig von der Framerate
				{
					if (CheckKey()==0) return(0);		//Das Keyboard abfragen
					Computer();							//Die Computerspieler
				}
				
				if (Testmodus) //Beim Testmodus immer den gesamten Bildschirm löschen
				{
					rcRectdes.left		= 0;
					rcRectdes.top		= 0;
					rcRectdes.right		= MAXX;
			...
			if (Bild%(LastBild/20+1) == 0)		//unabhänig von der Framerate
			...
			if (Bild%(LastBild/10+1) == 0)		//unabhänig von der Framerate
				CheckFXPixel();						//EffektPixel überprüfen
			lpDDSScape->Unlock(NULL);    
			
								
			if (AktMenue != -1)		//ein Overlaymenü vorhanden
			{
				...
				Menue[AktMenue].zeige();			//Menü anzeigen
			}
			else Zeige(true);							//Das Bild aufbauen
		}
		else if (Spielzustand == SZMENUE)
		{
			if (CheckMMenue() == 0) return 0;
			else continue;
		}
		else if (Spielzustand == SZSHOP)
		{
			for (i=0;i<MAXSPIELER;i++)
			{
				...
				if (Panzer[i].Computer)	ComputerShop(i); 
				else CheckShop(i);
			}
